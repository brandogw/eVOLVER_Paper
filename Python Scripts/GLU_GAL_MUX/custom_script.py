...
mixture = np.array(np.mat('7 1; 7 2; 7 3; 7 7; 6 1; 6 2; 6 3; 6 7; 5 1; 5 2; 5 3; 5 7; 4 1; 4 2; 4 3; 4 7'))



def valving (mux_command, vial_num = 0, media_type = [0], step_size = [5000], vials = range(0,16), x=1, elapsed_time = 99999, pump_wait = 1, exp_name = 'mux_test1', time_in = 1, wash = 3, preflush = 2000, logging = 'n'):
    ...
    if mux_command == 'flush': #set vials in function call in order to not wash all lines, otherwise range(0,16)
        ...
        for y in vials: #wash using peristaltic
            path = list(set(media[media_type[0]] + demux[y] + mux[y] + bridge + peri))
            bin_path = format(sum([binvals[i] for i in path]),'032b')
...
def precleaned_pump(vial_num, media_type, step_size, logging = 'y', x = 1, elapsed_time = 99999, pump_wait = 1, exp_name = 'mux_test1', time_in = 1): #use to combine commands for most common event
    valving('flush', vials = [vial_num], media_type = [7], x=x, elapsed_time = elapsed_time, pump_wait = pump_wait, exp_name = exp_name, time_in = time_in)
    valving('precleaned_syringe',vial_num,media_type,step_size, logging = logging, x=x, elapsed_time = elapsed_time, pump_wait = pump_wait, exp_name = exp_name, time_in = time_in)
    valving('efflux',vials = [vial_num], wash = 3, x=x, elapsed_time = elapsed_time, pump_wait = pump_wait, exp_name = exp_name, time_in = time_in, logging = logging)
    ...
    for x in vials:
        ...
        if len(data) > 15:
            ...
            if average_OD > ODset:
                ...
                step_size = (((-1)*np.log(lower_thresh[x]/average_OD)*volume)+3)/(0.002)

                if step_size > 5000:
                    step_size = 5000

                # send command to syringe pump 
                pump(vial_num = x, media_type = mixture[x], step_size = [step_size/2, step_size/2], logging = 'y', x=x, elapsed_time = elapsed_time, pump_wait = pump_wait, exp_name = exp_name, time_in = time_in)
